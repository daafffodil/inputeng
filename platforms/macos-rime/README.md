...
鼠须管扩展 v0.6.2。macOS 与 Windows 共用同一套优化核心：173,036 条简体中文词库、librime 原生双向离线释义、原生首次组合学习和一个轻量 DeepSeek 缺词过滤器；平台差异只保留在安装、外观设置、密钥存储和后台 worker。

发布包：`dist/macos/inputeng-macos-v0.6.2.zip`

## 架构

- Rime 的 engine、原生 translators/filters、词典与轻量 AI Lua filter 从 `platforms/windows/src/` 共用；打包时只去掉 Weasel 专用的候选窗 `style` 段，改由鼠须管全局外观补丁管理。
